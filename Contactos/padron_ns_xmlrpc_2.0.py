...
def get_state_id(state_name, country_id, models, db, uid, password):
    ...
    if state:
        ...
        print(f"Error: No se encontró el estado con el nombre '{state_name}' en el país {country_id.name}")
        return False
...
def get_branch_office_id(supplier_id, models, db, uid, password):
    ...
    branch_office = models.execute_kw(db, uid, password, 'branch.offices', 'search_read',
                                    [[('national_supplier_id', '=', supplier_id)]], {'fields': ['id']})
    if branch_office:
...
def get_layout_id(models, db, uid, password):
    ...
    layout_id = models.execute_kw(db, uid, password, 'bank.layouts.catalog', 'search_read', 
                                [[('census', '=', 'national_provider'), ('description', '=', 'Alta Beneficiarios'), ('states', '=', 'active')]], 
                                {'fields': ['id', 'bank_id']})
    if layout_id:
        return layout_id
    else:
...
def create_control_cuentas(layout_id, bank_account_id, supplier_id, bank_id, branch_office_id):
    for rec in layout_id:
        ...
        data_beneficiary_layout = {
            ...
            'national_supplier_id': supplier_id,
...
def change_ns_flag(partner_id):
    layout_id = models.execute_kw(db, uid, password, 'res.partner', 'search_read', 
                                ...
                                {'fields': ['id']})
    if layout_id:
        models.execute_kw(db, uid, password, 'res.partner', 'write', [[partner_id], {'flag_national_supplier': True}])
        print(f"El Contacto Padron con ID {partner_id} se ha marcado con NS_FLAG")


def change_ns_states(supplier_id):
    layout_id = models.execute_kw(db, uid, password, 'stantards.nationalsuppliers', 'search_read', 
                                [[('id', '=', supplier_id)]], 
                                {'fields': ['id']})
    if layout_id:
        models.execute_kw(db, uid, password, 'stantards.nationalsuppliers', 'write', [[supplier_id], {'states': 'active'}])
...
def process_suppliers(df):
    ...
    for index, row in df.iterrows():
        ...
        supplier_data = {
            ...
            'state': state_id,
            'city_hall': city_hall_id,
            'colony': row['Colonia'],
            'street': row['Calle'],
            'num_ext': row['Num. Ext.'],
            'num_int': num_int,
            'zip_code': row['Código Postal'],
            'states': 'in_review',
            'start_date': current_date,
            # Añadir aquí más campos según sea necesario
        }
        # Realizar validaciones o invocar métodos en Odoo
        print(f"Procesando proveedor: {supplier_data['name']}")
        ...
        try:
            ...
            data_contact_padron = {
                ...
                'type_beneficiary': 'national_supplier',
                'national_supplier_id': supplier_id,
                'start_date': current_date,
                'state': 'active'
            }
...
file_path = '/home/luanleon/Documentos/Padrones/Carga Inicial/IINGEN-UTICT-Padron-Proveedores_V2 - gfc 3 (ORIGINAL).xlsx'
